...
class SimpleValidationViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        viewModel = SimpleValidationViewModel(username: usrnameOutlet.rx.text.orEmpty.asObservable(), password: passwordOutlet.rx.text.orEmpty.asObservable())
        viewModel.usernameValid.bind(to: passwordOutlet.rx.isEnabled).disposed(by: rx.disposeBag)
        viewModel.usernameValid.bind(to: usernameValidOutlet.rx.isHidden).disposed(by: rx.disposeBag)
        viewModel.passwordValid.bind(to: passwordValidOutlet.rx.isHidden).disposed(by: rx.disposeBag)
        viewModel.everythingValid.bind(to: doSomethingOutlet.rx.isEnabled).disposed(by: rx.disposeBag)
        
        doSomethingOutlet.rx.tap.subscribe(onNext: {[weak self] in self?.showAlert()}).disposed(by: rx.disposeBag)
